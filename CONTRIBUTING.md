...
- Добавлять код для скачивания открытых датасетов, если они используются из папки
  `datasets`
- Убирать использование `tqdm` (он не стабилен в ноутбуках)
- Данные, которые скачиваются по ссылкам с dropbox и прочих ресурсов (относительно большие данные, которые нельзя загрузить в репозиторий), нужно скачать в специальную папку на гуглдиске чтобы не потерять эти данные.
