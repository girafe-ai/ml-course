...
class MDP:
    ...
    def get_next_states(self, state, action):
        ...
        assert action in self.get_possible_actions(state), "cannot do action %s from state %s" % (
            action,
            state,
        )
        ...
        for (row, col) in states:
            ...
            for action_i in range(len(actions)):
                ...
                for movement_i in [
                    ...
                    newrow, newcol = self.move(row, col, movement)
                    prob = (1.0 - slip_chance) if movement == action else (slip_chance / 2.0)
                    if prob == 0:
                        continue
