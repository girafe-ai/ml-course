...
class ImagePreprocessing(gym.ObservationWrapper):
    ...
    def __init__(self, env, width=84, height=84, grayscale=True):
        ...
        if self.grayscale:
            ...
            self.observation_space = spaces.Box(low=low, high=high, shape=obs_shape, dtype=dtype)
...
class MaxBetweenFrames(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        if isinstance(env.unwrapped, atari.AtariEnv) and "NoFrameskip" not in env.spec.id:
...
class TFSummaries(gym.Wrapper):
    ...
    def __init__(self, env, prefix=None, running_mean_size=100, step_var=None):
        ...
        self.step_var = step_var if step_var is not None else tf.train.get_global_step()
    ...
    def step(self, action):
        ...
        done_indices = [
            i for i, info in enumerate(info_collection) if info.get("real_done", done_collection[i])
        ]
