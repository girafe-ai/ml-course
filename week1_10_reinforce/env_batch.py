    ...
    def __init__(self, spaces):
        ...
        for space in spaces:
            if not isinstance(space, first_type):
                raise TypeError(
                    "spaces have different types: {}, {}".format(first_type, type(space))
                )
            if first_shape != space.shape:
                raise ValueError(
                    "spaces have different shapes: {}, {}".format(first_shape, space.shape)
                )
            if first_dtype != space.dtype:
                raise ValueError(
                    "spaces have different data types: {}, {}".format(first_dtype, space.dtype)
                )

        self.spaces = spaces
        super(SpaceBatch, self).__init__(shape=self.spaces[0].shape, dtype=self.spaces[0].dtype)
...
class ParallelEnvBatch(EnvBatch):
    ...
    def __init__(self, make_env, nenvs=None):
        ...
        self._processes = [
            Process(
                target=worker, args=(parent_connection, worker_connection, make_env), daemon=True
            )
            for i, (parent_connection, worker_connection, make_env) in enumerate(
                zip(self._parent_connections, self._worker_connections, make_env_functions)
            )
        ]
